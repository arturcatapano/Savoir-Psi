...
export default function Admin() {
  ...
  const [isPsiFixo, setIsPsiFixo] = useState(false) // Trava o filtro se for um psicólogo específico
  ...
  // --- MAPEAMENTO: E-MAIL -> NOME DO PROFISSIONAL ---
  const getPsiByEmail = (userEmail) => {
    ...
    return 'Todos' // Se for admin@savoir.com, vê tudo
  }

  // --- VERIFICA SE JÁ ESTÁ LOGADO (PERSISTÊNCIA) ---
  useEffect(() => {
    ...
    // Fica escutando mudanças no login/logout
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session) configurarUsuario(session.user)
  ...
  const configurarUsuario = (user) => {
    ...
    // Trava a visão dependendo do e-mail
  ...
  // --- LOGIN E LOGOUT REAL (SUPABASE AUTH) ---
  ...
  const abrirModalNovoAgendamento = () => {
    ...
    setFormManual({ 
      ...
      psicologa: isPsiFixo ? filtroPsi : '', // Se for psi logado, já preenche o nome dele
  ...
  const handleDeletarConfig = async (id) => {
    if (confirm('Deseja excluir este turno de atendimento?')) {
  ...
  // ========================================================================
  // INTERFACE DE LOGIN (Agora com E-mail)
  // ========================================================================
  if (!autenticado) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-savoir-navy text-white px-4">
        <form onSubmit={handleLogin} className="bg-white/10 p-8 rounded-2xl backdrop-blur-md w-full max-w-sm border border-white/20 shadow-2xl">
          ...
          <input 
            ...
            className="w-full p-3 rounded bg-white/90 text-gray-900 mb-3 focus:outline-none focus:ring-2 focus:ring-savoir-gold" 
            value={email} onChange={e => setEmail(e.target.value)} 
          />
          <input 
            type="password" placeholder="Sua Senha" required
            className="w-full p-3 rounded bg-white/90 text-gray-900 mb-6 focus:outline-none focus:ring-2 focus:ring-savoir-gold" 
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 flex flex-col md:flex-row relative">
      ...
      <aside className="w-full md:w-64 bg-savoir-navy text-white flex flex-col shadow-xl z-20 md:sticky md:top-0 md:h-screen">
        ...
        <nav className="flex-1 p-4 flex flex-col gap-2">
          <button onClick={() => setAbaAtiva('agenda')} className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${abaAtiva === 'agenda' ? 'bg-savoir-gold text-white' : 'hover:bg-white/10 text-gray-300'}`}>
            <LayoutDashboard size={20} /> Pacientes
          </button>
          <button onClick={() => setAbaAtiva('config')} className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${abaAtiva === 'config' ? 'bg-savoir-gold text-white' : 'hover:bg-white/10 text-gray-300'}`}>
            <Settings size={20} /> Meus Horários
          </button>
        </nav>

        <div className="p-4 border-t border-gray-700">
          <button onClick={handleLogout} className="flex items-center gap-3 p-3 w-full rounded-lg hover:bg-red-500/20 text-red-300 transition-colors">
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <h2 className="text-xl font-bold text-savoir-navy font-serif">
            {abaAtiva === 'agenda' ? 'Gestão de Agendamentos' : 'Configuração de Agenda'}
          </h2>
          <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            {abaAtiva === 'agenda' && (
              <button onClick={abrirModalNovoAgendamento} className="bg-savoir-navy text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-savoir-gold transition flex items-center gap-2 w-full md:w-auto justify-center">
                <Plus size={16}/> Novo Agendamento
              </button>
            )}
            
            {/* FILTRO CONDICIONAL: Só aparece habilitado se for o Admin Geral */}
            <div className="flex items-center gap-2 w-full md:w-auto">
              <Filter size={18} className="text-gray-400"/>
              <select 
                className="bg-gray-50 border border-gray-200 text-sm p-2 rounded-lg outline-none focus:border-savoir-gold w-full md:w-auto disabled:opacity-50 disabled:bg-gray-200" 
                value={filtroPsi} 
                onChange={e => setFiltroPsi(e.target.value)}
                disabled={isPsiFixo} // Trava se não for o Admin Geral
              >
        ...
        {abaAtiva === 'agenda' && (
          <div>
            {loadingAgendamentos ? (
              <p className="text-center py-20 text-gray-500">Carregando pacientes...</p>
            ) : listaAgendamentosFiltrada.length === 0 ? (
                  ...
                  <div key={item.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center hover:shadow-md transition">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${item.status === 'confirmado' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 md:mt-0 w-full md:w-auto">
                      <a href={`https://wa.me/55${item.telefone_paciente.replace(/\D/g,'')}?text=Olá ${item.nome_paciente}, sou da clínica Savoir Psi.`} target="_blank" className="flex-1 md:flex-none bg-green-50 text-green-600 p-2 rounded hover:bg-green-100 transition flex items-center justify-center border border-green-200" title="Chamar no WhatsApp">
                        <MessageCircle size={18} />
                      </a>
                      {item.status !== 'confirmado' && (
                        <button onClick={() => handleConfirmar(item.id)} className="flex-1 md:flex-none bg-blue-50 text-blue-600 p-2 rounded hover:bg-blue-100 transition flex items-center justify-center border border-blue-200" title="Confirmar">
                          <CheckCircle size={18} />
                        </button>
                      )}
                      <button onClick={() => abrirModalEditarAgendamento(item)} className="flex-1 md:flex-none bg-orange-50 text-orange-600 p-2 rounded hover:bg-orange-100 transition flex items-center justify-center border border-orange-200" title="Editar Data/Horário">
                        <Edit size={18} />
                      </button>
                      <button onClick={() => handleDeletarAgendamento(item.id)} className="flex-1 md:flex-none bg-red-50 text-red-500 p-2 rounded hover:bg-red-100 transition flex items-center justify-center border border-red-200" title="Excluir">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className={`md:col-span-1 p-6 rounded-xl shadow-sm border self-start transition-all ${editingConfigId ? 'bg-orange-50 border-orange-200' : 'bg-white border-gray-200'}`}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-savoir-navy flex items-center gap-2">
                  {editingConfigId ? <><Edit size={18} className="text-orange-500"/> Editar Turno</> : <><Plus size={18} className="text-savoir-gold"/> Cadastrar Turno</>}
                </h3>
                {editingConfigId && (
                  <button onClick={cancelarEdicaoConfig} className="text-xs text-red-500 font-bold hover:underline">Cancelar</button>
                )}
              </div>

              <form onSubmit={handleSalvarConfig} className="flex flex-col gap-4 text-sm">
                
                {/* Oculta a escolha do psicólogo se a pessoa já estiver logada com o perfil dela */}
                {!isPsiFixo && (
                  <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-gray-600 font-bold mb-1">Das:</label>
                    <input required type="time" className="w-full border p-2 rounded bg-white outline-none focus:border-savoir-gold" value={formConfig.hora_inicio} onChange={e => setFormConfig({...formConfig, hora_inicio: e.target.value})} />
                  </div>
                  <div>
                    <label className="block text-gray-600 font-bold mb-1">Até às:</label>
                    <input required type="time" className="w-full border p-2 rounded bg-white outline-none focus:border-savoir-gold" value={formConfig.hora_fim} onChange={e => setFormConfig({...formConfig, hora_fim: e.target.value})} />
                  </div>
                </div>
                <button type="submit" className={`w-full text-white font-bold py-3 rounded transition ${editingConfigId ? 'bg-orange-500 hover:bg-orange-600' : 'bg-savoir-navy hover:bg-savoir-gold'}`}>
                  {editingConfigId ? 'Atualizar Turno' : 'Salvar Turno'}
                </button>
              </form>
            </div>
            ...
            <div className="md:col-span-2">
              ...
              {loadingConfigs ? (
                <p className="text-gray-500 p-6 bg-white rounded-xl shadow-sm text-center">Carregando horários...</p>
              ) : listaConfigsFiltrada.length === 0 ? (
                 <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-dashed border-gray-300">
                  <p className="text-gray-400">Nenhum turno cadastrado.</p>
                </div>
              ) : (
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 relative">
            ...
            <form onSubmit={handleSalvarAgendamentoManual} className="flex flex-col gap-4">
              <div>
                ...
                <input required type="text" className="w-full border-b border-gray-300 p-2 outline-none focus:border-savoir-gold bg-transparent" value={formManual.nome_paciente} onChange={e => setFormManual({...formManual, nome_paciente: e.target.value})} />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">WhatsApp</label>
                <input required type="text" className="w-full border-b border-gray-300 p-2 outline-none focus:border-savoir-gold bg-transparent" value={formManual.telefone_paciente} onChange={e => setFormManual({...formManual, telefone_paciente: e.target.value})} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="date" className="w-full border-b border-gray-300 p-2 outline-none focus:border-savoir-gold bg-transparent" value={formManual.data_agendamento} onChange={e => setFormManual({...formManual, data_agendamento: e.target.value})} />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase">Horário</label>
                  <input required type="time" className="w-full border-b border-gray-300 p-2 outline-none focus:border-savoir-gold bg-transparent" value={formManual.horario} onChange={e => setFormManual({...formManual, horario: e.target.value})} />
                </div>
              </div>
              <button type="submit" className="mt-4 bg-savoir-gold text-white font-bold py-3 rounded-lg hover:bg-[#b09268] transition">
                {editingAgendamentoId ? 'Salvar Alterações' : 'Confirmar Agendamento'}
              </button>
            </form>
          </div>
        </div>
